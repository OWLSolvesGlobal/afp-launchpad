...
import { categorySlug, useCatalog } from "@/lib/catalog";
...
export const Header = ({ transparent = false }: { transparent?: boolean }) => {
  ...
  // Navigation is derived from the categories present in the Sheet —
  // women-first, already ordered by the snapshot. Never hardcoded.
  const nav = [
  ...
  return (
    ...
    >
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="fixed inset-0 z-50 bg-background text-foreground md:hidden animate-fade-up overflow-y-auto">
          ...
          <nav className="container flex flex-col gap-6 mt-12 pb-16">
            {nav.map((n) => (
              <Link
                key={n.to}
                to={n.to}
                className="font-serif text-4xl tracking-tight leading-none"
              >
                {n.label}
              </Link>
            ))}
